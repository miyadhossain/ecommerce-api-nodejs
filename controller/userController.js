...
const addToCart = asyncHandler(async (req, res) => {
  ...
  try {
    let products = [];
    const user = await User.findById(_id);
    // check if user already added in cart
...
const createOrder = asyncHandler(async (req, res) => {
  ...
  validateMongoDbId(_id);
  try {
    if (!COD) throw new Error("Create cash on delivery order failed");
    const user = await User.findById({ _id });
    ...
    let updatedCart = userCart.products.map((item) => {
      return {
        updateOne: {
    ...
    const updatedProduct = await Product.bulkWrite(updatedCart, {});
    res.json({ message: "The Order placed is success!!!" });
  } catch (error) {
    throw new Error(error);
  }
});
